...
describe("std::agency walks comprehension contents", () => {
  it("finds a call written inside a comprehension body", () => {
    const src = ["node main() {", "  const r = [double(x) for x in xs]", "}"].join("\n");
  ...
  it("finds a call in the iterable and in the filter", () => {
    const src = ["node main() {", "  const r = [x for x in getItems() if keep(x)]", "}"].join("\n");

    const names = (_getNodesOfType(src, ["functionCall"]) as any[]).map((c) => c.functionName);
