...
**Follow-up fixes — block expressions & block-local function calls (now resolved).** Investigation during review surfaced two pre-existing, related bugs that this PR now also fixes:

1. **Block-expression interiors weren't scope-resolved.** A `\… -> …` lambda passed as a named-arg / positional *value* (e.g. `.partial(f: \-> y)`) or assigned to a variable was never descended into by `walkNodes` (only trailing `as {}` blocks were), so a reference to an *enclosing* block's variable emitted a bare, unscoped identifier (`ReferenceError`). The same lambda as a *trailing block* (`callIt(\-> y)`) already worked, which initially masked the bug. Fixed by adding a `blockArgument`-as-expression descent to `walkNodes` so `resolveBlockScopes` visits those interiors. (Note: a *multi-statement* block body is still not parseable as a block expression — `\-> { … }` fails to parse — so a nested `as {}` block literally inside a `.partial` value is not expressible.)

2. **Calling a block-local function variable was broken.** A function held in a block `let` (e.g. a `.partial(...)` result) emitted a bare callee (`__call(bound, …)` → `ReferenceError`) because functionCall callee scope was resolved only via `lookupScope`, which doesn't know block locals. Fixed by resolving functionCall callees against the block chain in `resolveBlockScopes` and threading `blockDepth` to the callee codegen site.

Both fixes carry their own execution tests (`block-expr-partial-capture`, `block-local-fn-call`, `block-local-fn-call-nested`) and leave the full unit suite green with no fixture changes. The depth-agreement reasoning above holds for blocks reached through `functionCall.block`, block expressions, and `fork`.
