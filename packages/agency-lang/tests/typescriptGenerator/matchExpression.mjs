...
const __globalCtx = new RuntimeContext({
  ...
  logLevel: "info",
  traceConfig: {
    program: "matchExpression.agency"
  }
});
...
async function __initializeGlobals(__ctx) {
  if (__ctx.globals.isInitialized("matchExpression.agency")) {
    return;
  }
  __ctx.globals.markInitialized("matchExpression.agency")
}
__registerGlobalsInit("matchExpression.agency", __initializeGlobals);
async function __registerTopLevelCallbacks(__ctx) {

}
__registerCallbacksInit("matchExpression.agency", __registerTopLevelCallbacks);
...
let __functionCompleted = false;
  claimFrameForScope(__stack, "main", "matchExpression.agency");
...
if (__process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    ...
    const __result = await runCliEntry({
      nodes: {
        main: () => main(undefined, initialState)
      },
