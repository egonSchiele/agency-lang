...
- `docs/dev/terminal-status.md` — Naming the terminal tab and driving its progress indicator: the title-stack and ConEmu OSC 9;4 sequences, the four quiet conditions and the `AGENCY_TERM_STATUS_OWNED` one-owner rule for nested processes, why the wiring lives in `runCli` and the signal handlers never exit, and the deliberately sticky red tab on failure
