...
async function __initializeGlobals(__ctx) {
  if (__ctx.globals.isInitialized("castUnchecked.agency")) {
    return;
  }
  __ctx.globals.markInitialized("castUnchecked.agency")
}
__registerGlobalsInit("castUnchecked.agency", __initializeGlobals);
async function __registerTopLevelCallbacks(__ctx) {

}
__registerCallbacksInit("castUnchecked.agency", __registerTopLevelCallbacks);
__functionRefReviver.registry = __toolRegistry;
const Person = z.object({ "name": z.string() });
type Person = z.infer<typeof Person>;
async function __greet_impl(p: Person) {
...
let __functionCompleted = false;
  claimFrameForScope(__stack, "greet", "castUnchecked.agency");
  if (!__globals()!.isInitialized("castUnchecked.agency")) {
    await __initializeGlobals(__ctx)
  }
  let __funcStartTime: number = performance.now();
  __stack.args["p"] = p;
  __self.__destructiveRan = __self.__destructiveRan ?? false;
  const runner = new Runner(__ctx, __stack, { state: __stack, moduleId: "castUnchecked.agency", scopeName: "greet", threads: __setupData.threads });
...
await callHook({
          ...
          data: {
            ...
            moduleId: "castUnchecked.agency"
          }
        })
      });
      await runner.step(1, async (runner) => {
__functionCompleted = true;
runner.halt(__stack.args.p.name)
return;
      });
    })
    if (runner.halted) {
...
let __forked;
let __functionCompleted = false;
  claimFrameForScope(__stack, "main", "castUnchecked.agency");
  const runner = new Runner(__ctx, __stack, { nodeContext: true, state: __stack, moduleId: "castUnchecked.agency", scopeName: "main", threads: __setupData.threads });
  try {
    await agencyStore.run({
      ...getRuntimeContext(),
      ctx: __ctx,
...
__stack.locals.p = __stack.locals.raw;
      });
      await runner.step(3, async (runner) => {
runner.halt({
          messages: __threads(),
          data: await __call(greet, {
            type: "positional",
            args: [__stack.locals.raw]
          })
        })
return;
      });
...
export const __sourceMap = {"castUnchecked.agency:greet":{"1":{"line":3,"col":2}},"castUnchecked.agency:main":{"1":{"line":7,"col":2},"2":{"line":8,"col":2},"3":{"line":9,"col":2}}};