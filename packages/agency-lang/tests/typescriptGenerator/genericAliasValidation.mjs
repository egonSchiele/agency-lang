...
__registerCallbacksInit("genericAliasValidation.agency", __registerTopLevelCallbacks);
...
let __functionCompleted = false;
  claimFrameForScope(__stack, "process", "genericAliasValidation.agency");
...
if (isAborted(__funcResult)) {
          runner.halt(__funcResult.carryThrough(__stack, "process"))
          return;
        }
      });
...
if (__error instanceof AgencyAbort) {
  ...
  return AbortedResult.fromError(__error, __stack, "process");
}
...
let __forked;
let __functionCompleted = false;
  claimFrameForScope(__stack, "main", "genericAliasValidation.agency");
