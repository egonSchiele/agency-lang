...
| Comment | File:Line | Plan | Status |
|---|---|---|---|
| #26 | `lib/runtime/memory/types.ts:13` | Replace `MemoryMessage` with `smoltalk.Message`. Touches: `extraction.ts`, `compaction.ts`, `manager.ts`, `llmAdapter.ts`. `prompt.ts` no longer needs `toMemoryMessages` — it can pass `messages.getMessages()` directly. The `role` filter in `compactIfNeeded` already works on `smoltalk.Message.role`. | TODO (carried) |
