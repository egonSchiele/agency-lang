 ...
 * yielding their inner expressions with their own scope chain. When
 * we're checking a single scope, we want to skip items that belong to
 * nested scopes — they get checked separately when their own ScopeInfo
 * is processed. Without this filter, the global pass would re-check
 * expressions inside `node main()` against the global scope, which can
 * surface false positives where a function parameter or local shadows
 * a global.
 *
 * Items inside intra-def constructs that don't open a new scope (if /
 * while / for / handle / fork bodies) keep the parent's scope chain,
 * so they pass through.
 *
 * walkNodes seeds an empty scopes list with [globalScope]. For a
 * per-def ScopeInfo, the body passed to walkNodes is the def's body,
 * so direct children appear with scopes=[globalScope] — accept those
 * when info is a per-def scope.
 *
...
function isClassMethodScope(key: string): boolean {
  // walkNodes tags method bodies with `function:Class.method` (note
  // the `.` between class and method names). Function defs use plain
