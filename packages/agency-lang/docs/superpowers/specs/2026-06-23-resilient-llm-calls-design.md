...
> **Classification is provider-neutral, status-first, message-fallback.** The LLM client is swappable (`lib/runtime/llmClient.ts` — `ctx.llmClient`), so retry classification must NOT import a provider SDK. The client adapter translates its errors into a neutral `NormalizedLLMError { status?, retryAfterMs?, kind?, message }`; agency's classifier reads only those. For smoltalk 0.4.2, `SmoltalkClient.normalizeError` maps `SmolError.status`/`headers` and the typed subclasses (`SmolContentPolicyError`→`contentPolicy`, etc.) into that shape. Classification then reads `status` (429→`rateLimit` with `retryAfterMs`, 529→`overloaded`, 5xx→`serverError`, other 4xx→terminal) and `kind` (terminal); status-less transport drops fall back to **message-matching** for `connectionLost`/`streamInterrupted`. smoltalk's response headers are an allowlist (most stripped for security), but `retry-after` / `retry-after-ms` / `x-ratelimit-*` are kept, so `retryAfterMs` is available (raw headers on `err.cause` if ever needed). A custom client that doesn't implement `normalizeError` degrades cleanly to message-matching.
...
1. **smoltalk error surface** — RESOLVED (smoltalk 0.4.2 exposes `status`/`headers`/`cause`; headers are an allowlist that keeps `retry-after`/ratelimit). Classification is **abstracted** behind `NormalizedLLMError` (the swappable client adapter populates it), so it never imports smoltalk and survives a client swap. Agency catches no raw provider `APIError`s, so smoltalk's "wrap unclassified APIError as SmolError" behavior change is safe.
