...
async function _url(__metadata) {
  ...
  try {
    ...
    const extracted =  __extractResponse(__rawResult, z.object({ "hostname": z.string(), "port": z.number() }));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  } catch (e) {
    const extracted =  __extractResponse(responseMessage.output, z.object({ "hostname": z.string(), "port": z.number() }));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  }
