...
function forest(): TreeNode[] {
  const llmA = span("llmCall", [
    leaf("promptCompletion", 30_000, { model: '"m1"', threadId: "5", timeTaken: 29_000,
      messages: [{ role: "user", content: "q" }] }),
  ], { id: "a" });
  const llmB = span("llmCall", [
  ...
  return [trace([
    leaf("threadCreated", 0, { threadId: "5", label: "codingAgent" }),
    llmA, llmB, bash,
  ])];
}
...
describe("ByNameView", () => {
  ...
  it("Enter hands the kernel group key to occurrences", () => {
    const view = new ByNameView(forest(), "T", DEFAULT_THRESHOLDS);
    expect(view.handleKey({ key: "enter" }, viewport))
      .toEqual({ kind: "openOccurrences", groupKey: "llm(codingAgent)" });
  });

  it("d opens the detail of the group's LONGEST call", () => {
    const view = new ByNameView(forest(), "T", DEFAULT_THRESHOLDS);
    expect(view.handleKey({ key: "d" }, viewport))
      .toEqual({ kind: "openDetail", spanId: "a" });   // 29s beats 10s
  });
