...
function forest(): TreeNode[] {
  ...
  const root = span("agentRun", [leaf("agentStart", 0, { entryNode: "main" }), agent], { id: "root" });
  return [trace([root])];
}
...
describe("OccurrencesView", () => {
  ...
  it("Enter on a leaf opens detail; on a call with children opens a drilled flame", () => {
    ...
    expect(view.handleKey({ key: "enter" }, viewport))
      .toEqual({ kind: "openDetail", spanId: "b1" });
    view.handleKey({ key: "down" }, viewport);
    expect(view.handleKey({ key: "enter" }, viewport))
      .toEqual({ kind: "openFlameAt", spanId: "b2" });
  });
  ...
  it("the prefix cut lands on a segment boundary, never mid-name", () => {
    ...
    const helper = span("toolExecution", [
    ...
    const root = span("agentRun", [leaf("agentStart", 0, { entryNode: "main" }), helper, agent], { id: "root" });
  ...
  it("a vanished group after setData notes it and backs out on the next key", () => {
    const view = new OccurrencesView(forest(), "T", "bash", DEFAULT_THRESHOLDS);
    const empty = [trace([span("agentRun", [leaf("agentStart", 0, { entryNode: "main" })], { id: "root" })])];
    view.setData(empty);
