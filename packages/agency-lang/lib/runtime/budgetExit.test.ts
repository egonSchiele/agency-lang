...
import { formatBudgetExceeded, reportBudgetExceededAndExit } from "@/runtime/budgetExit.js";
...
describe("formatBudgetExceeded", () => {
  test("cost message", () => {
    expect(formatBudgetExceeded({ dimension: "cost", limit: 0.5, spent: 0.63 })).toBe(
      "Exceeded cost limit of $0.5 (used $0.63)",
    );
  });
  test("time message renders whole ms", () => {
    expect(formatBudgetExceeded({ dimension: "time", limit: 5000, spent: 5002.4 })).toBe(
