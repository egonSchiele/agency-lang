...
export class RuntimeContext<T> {
  ...
  // Stores interrupt responses and data for resume, keyed by interrupt_id.
  // Each entry has { response: InterruptResponse, interruptData?: InterruptData }
  interruptResponses?: Record<string, { response: any; interruptData?: any }>;
  ...
  */
  ...
  /** Look up the interrupt data for a given interrupt_id, with the response merged in.
   *  Returns an InterruptData object ready to be assigned to __state.interruptData. */
  getInterruptData(interruptId: string): any | undefined {
    const entry = this.interruptResponses?.[interruptId];
    console.log("[getInterruptData]", interruptId, "entry:", entry ? "found" : "NOT FOUND", "keys:", Object.keys(this.interruptResponses || {}));
    if (!entry) return undefined;
    return {
      ...entry.interruptData,
      interruptResponse: entry.response,
    };
  }
