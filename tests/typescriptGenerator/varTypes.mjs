...
async function _response(person, __metadata) {
  ...
  try {
    ...
    const extracted =  __extractResponse(__rawResult, z.object({ "greeting": z.string() }));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  } catch (e) {
    const extracted =  __extractResponse(responseMessage.output, z.object({ "greeting": z.string() }));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  }
