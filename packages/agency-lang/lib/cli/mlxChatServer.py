...
past a count is cut short too, as is one that repeats a whole sentence a few
times. Without these, a small model that talks itself in circles runs to
`max_tokens`, which on a large model is minutes of the machine for nothing.
...
the end-of-reply token is allowed.

Each limit can be set per request, in the body: `reasoning_budget`,
`hedge_limit`, and `repeat_limit`, each a count with 0 meaning no limit. A
request that names none gets the server's flags, `--reasoning-budget`,
`--hedge-limit`, and `--repeat-limit`. The reasoning budget defaults to half
the request's `max_tokens`, so a reply always has room left to answer.
...
the rest) plus the three limit flags above. Distributed serving
(`mlx.distributed`) is not supported; the script always runs the HTTP server
in its own process.
"""
...
@dataclass
class Limits:
    """How far a reply may go before it is cut short. Each is a count, and 0
    means no limit. A reasoning budget of None means half the request's
    `max_tokens`, decided per request."""
...
SERVER_LIMITS = Limits(reasoning_budget=None, hedge_limit=12, repeat_limit=3)

# A reply is judged every this many tokens, not every token: the scans below
# read the whole reply so far.
CHECK_EVERY = 16

# The phrases a model uses when it doubts what it just wrote. One of these
# now and then is thinking; a dozen is a loop.
HEDGES = re.compile(
    r"\b(?:But wait|Wait|Hmm+|Hold on|Actually,? wait"
    r"|Let me (?:reconsider|re-?check|double-?check|re-?examine|re-?read|verify|think again))\b"
)
...
def limits_for(body, max_tokens, defaults):
    ...
    flags. Raises ValueError for a field that is not a count, which the
    ...
    budget = field("reasoning_budget", defaults.reasoning_budget)
    if budget is None:
        budget = max_tokens // 2
    return Limits(
        reasoning_budget=budget,
        hedge_limit=field("hedge_limit", defaults.hedge_limit),
        repeat_limit=field("repeat_limit", defaults.repeat_limit),
    )
...
class Watcher:
    ...
    def __init__(self, tokenizer, ll_tokenizer_for, token_masks_for, grammar, initial_state, limits):
        ...
        self.broken = False
        # The text before this offset has been judged already: the answer is
        # judged on its own, not with the thinking before it, and a JSON
        # field cut short does not count against the fields after it.
        self.judged_from = 0
    ...
    def consume(self, token_id):
        ...
        if self.matcher is None or self.broken:
            return
    ...
    def enter_answer(self):
        self.phase = "answer"
        self.closing = None
        self.judged_from = len(self.detokenizer.text)
    ...
    def step(self, token_id):
        ...
        if self.phase == "start":
            if token_id == self.think_start[0]:
                ...
                self.enter_answer()
            self.detokenizer.add_token(token_id)
            if self.phase == "answer":
                self.consume(token_id)
        elif self.phase == "reasoning":
            self.detokenizer.add_token(token_id)
            self.reasoning_tokens += 1
            if self.closing is not None and token_id == self.think_end[self.closing]:
                self.closing += 1
    ...
    def check(self):
        ...
        if self.phase == "reasoning":
            ...
            if budget and self.reasoning_tokens >= budget:
                self.cut_short(f"has thought for {self.reasoning_tokens} tokens, its budget")
                return
        elif self.ending:
            return
        text = self.detokenizer.text[self.judged_from :]
    ...
    def __call__(self, tokens, logits):
        ...
        if self.phase == "reasoning":
            if self.closing is not None:
                only(self.mask, [self.think_end[self.closing]])
...
class Generator(server.ResponseGenerator):
    ...
    def generate(self, request, args, progress_callback=None):
        """mlx_lm's `generate`, with one change: while the handler waits for
        ...
        client is gone when it writes the finished reply.
