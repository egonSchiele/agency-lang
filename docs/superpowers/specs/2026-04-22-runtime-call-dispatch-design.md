...
Blocks are compiled into async functions and included in the descriptor's `args` array. When `__call` spreads `descriptor.args` for a TS function, the block ends up as the last positional argument. No special handling needed.
