...
describe("argv builders — reads", () => {
  ...
  it("logArgs rejects a flag-shaped ref", () => {
    expect(() =>
      logArgs({ count: 5, oneline: false, path: "", ref: "--output=x", author: "" }),
    ).toThrow();
  });
  ...
  it("showArgs", () => {
    expect(showArgs({ ref: "HEAD" })).toEqual([
      "show",
      "--patch",
      "-M",
      "--end-of-options",
  ...
  it("blameArgs uses --line-porcelain, hardens path/ref, no --end-of-options (git blame rejects it before --)", () => {
    expect(blameArgs({ path: "a.ts", ref: "" })).toEqual([
      "blame",
      "--line-porcelain",
      "--",
      "a.ts",
    ]);
    expect(blameArgs({ path: "a.ts", ref: "HEAD" })).toEqual([
      "blame",
      "--line-porcelain",
      "HEAD",
      "--",
...
describe("argv builders — writes", () => {
  ...
  it("commitArgs passes the message as the value of -m; rejects empty", () => {
    expect(commitArgs({ message: "--amend looking msg" })).toEqual([
      "commit",
      "-m",
      "--amend looking msg",
    ]);
  ...
  it("checkoutArgs / switchArgs", () => {
    expect(checkoutArgs({ target: "main", force: false })).toEqual([
      "checkout",
      "--end-of-options",
      "main",
    ]);
    expect(checkoutArgs({ target: "main", force: true })).toEqual([
      "checkout",
      "--force",
      "--end-of-options",
      "main",
    ...
    expect(switchArgs({ branch: "x", create: true })).toEqual([
      "switch",
      "-c",
      "--end-of-options",
      "x",
  ...
  it("branchCreateArgs / branchDeleteArgs (force + protected)", () => {
    ...
    expect(branchDeleteArgs({ branch: "x", force: false, protectedBranches: [] })).toEqual([
      "branch",
      "-d",
      "--end-of-options",
      "x",
    ]);
    expect(branchDeleteArgs({ branch: "x", force: true, protectedBranches: [] })).toEqual([
      "branch",
      "-D",
      "--end-of-options",
      "x",
    ...
    expect(() =>
      branchDeleteArgs({ branch: "main", force: true, protectedBranches: ["main"] }),
    ).toThrow(/protected/);
  });
  it("stashPushArgs / stashPopArgs / restoreArgs", () => {
    ...
    expect(restoreArgs({ paths: ["a.ts"], staged: true })).toEqual([
      "restore",
      "--staged",
      "--",
      "a.ts",
