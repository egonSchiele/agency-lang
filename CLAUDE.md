...
- `docs/dev/statelog-clients.md` — The five sealed statelog CLI clients and `readJsonBody`: why every response body goes through the shared reader, the redirect/http:// diagnosis it prints for non-JSON responses, and the test rule that mocks stub `text()` not `json()`
