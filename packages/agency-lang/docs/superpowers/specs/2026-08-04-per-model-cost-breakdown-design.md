...
- Tokens: `sum(models[*].inputTokens) + unattributed.inputTokens === inputTokens` (exact — integers, within the safe-integer range; see the count-validation note above). Same for output.
...
export type InvocationUsage = {
   ...
   *  USAGE_RECONCILE_TOLERANCE_USD (cost) / exactly for tokens within the
 *  safe-integer range. Null-prototype so
