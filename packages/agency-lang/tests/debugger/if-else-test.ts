...
__toolRegistry["readSkill"] = __AgencyFunction.create({
  name: "readSkill",
  module: "tests/debugger/if-else-test.agency",
  fn: readSkill,
...
let __functionCompleted = false;
  const runner = new Runner(__ctx, __stack, { nodeContext: true, state: __stack, moduleId: "tests/debugger/if-else-test.agency", scopeName: "main", stack: __stateStack, threads: __threads });
  if (!__state.isResume) {
...
export async function main(x: number, { messages, callbacks }: { messages?: any; callbacks?: any } = {}): Promise<RunNodeResult<any>> {
  return runNode({
    ctx: __globalCtx,
    nodeName: "main",
    data: {
