...
- `docs/dev/doc-cache.md` — The `agency doc` incremental cache: the output-directory ownership ledger, freshness vs ownership evidence, the linkTargets re-check, why `@hidden` must drop out of the symbol registry (and why that needs no render-key bump), the deletion boundary, and the conservative no-ledger contract
