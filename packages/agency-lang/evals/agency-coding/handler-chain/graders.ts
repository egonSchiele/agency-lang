...
(pass for "no opinion"). (3) Both handlers decide with a match on data.effect,
and the outer one reads the count from the interrupt data (data.data.count).
...
Code that is not valid Agency scores 0 whatever else it does: forms like
