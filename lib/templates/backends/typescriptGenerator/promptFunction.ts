...
async function _{{{variableName:string}}}({{{argsStr:string}}}) {
  ...
  const __clientConfig = __getSmoltalkConfig({{{clientConfig:string}}});
  let responseMessage;
  ...
  try {
    ...
    const extracted =  __extractResponse(__rawResult, {{{zodSchema:string}}});
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  } catch (e) {
    const extracted =  __extractResponse(responseMessage.output, {{{zodSchema:string}}});
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  }
  {{/hasResponseFormat}}
