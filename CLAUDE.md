...
- `docs/dev/statelog-to-dataset.md` — Labeling a statelog trace into a dataset (viewer `l` + `agency label ingest <statelog> --trace`): a trace is just an example, so it reuses the shared output selection (`selectLabelingFinalOutput`) and projection (`projectOccurrenceFields`, shared with `loadRun`); the minimal `statelog` occurrence origin `{traceId, finalOutputIndex}`; the shared datasetWriter/labelingHost effect owners and sequential lock ordering; and the viewer hook (`l` labels, never expands) + focusItem
...
- `docs/dev/agent-brains.md` — The agency agent's harness/brain split: the `AgentBrain` record, the harness `runTurn` pipeline (budget → slash expansion → attachments on the main target only → brain), `runSession` as the only place a brain is called and why that keeps the policy handler outermost, the bundled-reads-stay-static rule for brain modules and its two reasons, `--brain` selection, how to add a brain, and the two compiler bugs worked around
