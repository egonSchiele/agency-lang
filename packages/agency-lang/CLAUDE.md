...
- `docs/dev/stdlib/ocr.md` — `std::ocr` and `viewFile`: the three effects and why they are separate, the before/after-approval split and where each function reads its bytes, the osascript `-e` bridge with a temp copy, the Vision y flip, why local never falls back to cloud, and why `viewFile` says "queued".
