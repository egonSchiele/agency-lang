...
The `tics-*` tests, and the older tic tests `also-answers-to`, `costs-no-parse`, and `correction-before-checking`, plant verbal tics from the reviewer's tics pass (`stdlib/agents/writing/tics.agency`). Cuts and tics come back as advisory findings, so `harvestedGraders()` has no `rejects` grader; the editor's points are the ground truth. `rejects` stays on the planted-flaw tests. `tics-control` is plain prose on the same kind of subject; a reviewer that flags every colon or "only" fails its `rejects-nothing` grader.
