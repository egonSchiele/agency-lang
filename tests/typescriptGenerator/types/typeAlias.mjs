...
async function _foo(__metadata) {
  ...
  try {
    ...
    const extracted =  __extractResponse(__rawResult, z.object({ "x": z.number(), "y": z.number() }));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  } catch (e) {
    const extracted =  __extractResponse(responseMessage.output, z.object({ "x": z.number(), "y": z.number() }));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  }
