...
>
> **Done:**
> - Tasks 1–3 (codegen-emitted hook sites). `onNodeStart` / `onNodeEnd` / `onFunctionStart` / `onFunctionEnd` now correctly halt the enclosing runner with the failure when a callback's interrupt is rejected. Covered by `tests/agency/callback-rejection-halts-function`.
> - Task 4 (LLM hook sites). `_runPrompt` now fires `onLLMCallStart` / `onLLMCallEnd` through `pr.fireHook` instead of `callHook`. `pr.fireHook` is a new `PromptRunner` method that mirrors `Runner.hook`'s substrate: `runBatch` with one sequential child per callback. The per-callback branches preserve each callback's frame across resume via `runBatch`'s shared checkpoint + cached-branch short-circuit. On resume the interrupted callback's frame is restored from the snapshot in deserialize mode, the codegen `if (__response)` short-circuit consumes the user's reject response, and the failure propagates through `_runPrompt` → `runPrompt` → user `llm()` site as a `Failure` value. Covered by `tests/agency/callback-rejection-aborts-llm`.
>
> **Deferred:**
> - Task 5 (tool hook sites). `onToolCallStart` / `onToolCallEnd` fire inline via `invokeCallbacks` on the tool branch's stack rather than through a per-callback `runBatch`. When a tool callback halts (interrupt + reject), the callback's frame is popped from the tool branch's stack in the generated `finally`, and the surrounding `pr.parallel` uses `recordBranchOutcomes: false` so `runBatch` never stamps the tool branch with the callback's leaf checkpoint — the snapshot serializes an empty branch stack and on resume the callback re-fires from a fresh frame, raising a brand-new interrupt instead of consuming the saved response. The existing `multi-tool-callback-interrupts-*` fixtures dodge this with `if (firedA == 1)` guards that prevent infinite interrupt loops; my new `callback-rejection-rejects-tool-call` fixture hit the bug directly and was removed pending the fix. Two possible fixes for a follow-up PR: (a) have the tool branch body call `stack.setInterruptOnBranch(branchKey, ..., outcome.interrupts[0].checkpoint)` so the leaf CP becomes the branch's vehicle (analogous to how `pr.fireHook` works), or (b) add a `b.fireHook` to `BranchRunner` that uses a nested `runBatch` with per-callback children. Once fixed, the runtime code for routing tool callback failures into a rejection `toolMessage` + `deleteBranch` (originally part of this PR, now removed for being dead code) can land alongside.
