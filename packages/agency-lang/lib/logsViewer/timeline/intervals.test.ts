...
describe("subtract", () => {
  ...
  it("subtracts the UNION of overlapping pieces (parallel fork children)", () => {
    expect(subtract({ start: 0, end: 100 }, [{ start: 10, end: 50 }, { start: 30, end: 70 }]))
      .toEqual([{ start: 0, end: 10 }, { start: 70, end: 100 }]);
  });
  ...
  it("clamps pieces that extend past the base — malformed logs must not go negative", () => {
    const out = subtract({ start: 10, end: 20 }, [{ start: 0, end: 15 }, { start: 18, end: 99 }]);
...
describe("coverage", () => {
  it("full-window interval covers every cell fully", () => {
    expect(coverage([{ start: 0, end: 100 }], { start: 0, end: 100 }, 4))
      .toEqual([1, 1, 1, 1]);
  });

  it("half-cell occupancy reports 0.5 for that cell", () => {
    expect(coverage([{ start: 0, end: 12.5 }], { start: 0, end: 100 }, 4)[0])
      .toBeCloseTo(0.5);
  });
  ...
  it("intervals outside the window contribute nothing", () => {
    expect(coverage([{ start: 200, end: 300 }], { start: 0, end: 100 }, 4))
      .toEqual([0, 0, 0, 0]);
  });
...
describe("totalMs", () => {
  it("sums interval lengths", () => {
    expect(totalMs([{ start: 0, end: 3 }, { start: 10, end: 14 }])).toBe(7);
  });
});
