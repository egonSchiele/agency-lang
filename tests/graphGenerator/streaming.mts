...
async function _response(__metadata?: Record<string, any>): Promise<string> {
  const __prompt = `Generate a response word by word`;
...
async function _response2(__metadata?: Record<string, any>): Promise<string> {
  const __prompt = `Generate a response word by word, but with a different model`;
  const startTime = performance.now();
  let __messages: Message[] = __metadata?.messages || [];
...
export async function foo({ messages, callbacks } = {}): Promise<__State<any>> {
