...
**Known limits:** result-pattern bindings (`is success(v)` binds `v`) and match-arm pattern binders are not tracked — both are branch-scoped bindings, which need flow-aware rename planning; the guide documents them.
