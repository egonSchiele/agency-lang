...
// A coerced flag-value map: declared flag name → final typed value.
// Used both as the user-facing `ParsedArgs.flags` shape and as the
// pipeline-internal value type from `coerceValues` onward.
export type FlagValues = Record<string, string | number | boolean>;

// Pre-coerce values as returned by `node:util.parseArgs`: numbers are
// still raw strings, booleans are booleans.
export type RawFlagValues = Record<string, string | boolean>;

// Required flag-group constraints (same fields as `FlagGroups`, but
// with absent groups normalized to `[]`).
export type NormalizedGroups = Required<FlagGroups>;
...
//
// Derived from `FlagSpec` so the field list lives in exactly one place.
// The four non-nullable fields (`type`, `required`, `description`,
...
export function normalizeSchema(schema: ArgsSchema): NormalizedSchema {
  ...
  return {
    ...
    flagsByShort: indexByOptional(flags, (f) => f.short),
    groups: {
...
function defaultFor(spec: FlagSpec): string | number | boolean | null {
...
export function callNodeParse(
  ...
  try {
    ...
    return {
      ok: true,
      value: {
        flags: out.values as RawFlagValues,
...
function buildNodeOptions(schema: NormalizedSchema): NodeOptionMap {
  const out: NodeOptionMap = {};
  for (const f of schema.flags) {
    const entry: NodeOptionMap[string] = {
...
function buildResult(
  ...
  const visible = Object.create(null) as FlagValues;
