...
**Known limitation — standalone block expressions (`processBlockAsExpression`).** A block used as an *expression* (a `\… -> …` lambda passed as a `.partial(...)` named arg or assigned to a variable) DOES push a builder block scope, but `walkNodes` has **no `blockArgument`-as-expression descent** (verified in `lib/utils/node.ts`), so neither the old Phase-1 loop nor the new `resolveBlockScopes` ever visits such a block's interior. A reference to an *enclosing* block's variable from inside a block expression therefore gets **no scope set** and is emitted as a **bare, unscoped identifier** — already broken on `main` (empirically: `\-> y` referencing an enclosing block local generates `runner.halt(y)`, a `ReferenceError`). This is **pre-existing** and entirely **unchanged** by this work: this PR's reads/writes only differ when `resolveBlockScopes` assigns `block`/`blockArgs` scope, which it never does inside block expressions. Note also that a *multi-statement* block body (the only place a nested `as {}` block could live) is **not even parseable** as a block expression — `\-> { … }` fails to parse — so "a nested block inside a `.partial` block" is not currently expressible. Extending coverage to block expressions would require adding a `blockArgument`-as-expression descent to `walkNodes` and is a separate change, out of scope here. The depth-agreement reasoning above holds for blocks reached through `functionCall.block` and `fork`, which is where the reported bug lives.
