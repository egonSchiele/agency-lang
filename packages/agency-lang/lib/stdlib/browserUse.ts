...
async function pollSession(
  ...
  throw new Error(`Browser Use session ${sessionId} timed out after ${timeout / 1000}s`);
}
...
async function browserUseImpl(
  ...
  if (options?.proxyCountryCode) body.proxyCountryCode = options.proxyCountryCode;
