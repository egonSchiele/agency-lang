...
describe("_read offset/limit", () => {
  ...
  it("returns the whole file when no offset/limit and under cap", async () => {
  ...
  it("appends a truncation note when file exceeds the soft cap", async () => {
    const big = Array.from({ length: 3000 }, (_, i) => `L${i + 1}`).join("\n");
    writeFileSync(join(dir, "big.txt"), big, "utf-8");
    const out = await _read(dir, "big.txt");
    expect(out).toContain("[truncated: showing 1-2000 of 3000 lines");
  });
