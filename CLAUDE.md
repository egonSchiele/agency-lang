...
- `docs/dev/terminal-status.md` — Naming the terminal tab and driving its progress indicator: the title-stack and ConEmu OSC 9;4 sequences, the four quiet conditions and the `AGENCY_TERM_STATUS_OWNED` one-owner rule for nested processes, why there is deliberately NO `SIGINT` handler (a JS listener defers the signal to the event loop and would break instant Ctrl-C on synchronous commands; and a signal is not a verdict — eval's first Ctrl-C is non-fatal), `commandFailed`'s 130/143-are-not-failures rule, and the deliberately sticky red tab
