...
The full position analysis lives in the header comment of `lib/utils/expressionSlots.ts`, next to the table it describes.
...
The type checker refuses a checked cast to a tagged type wherever the pass does not lift it. `lib/preprocessors/hoistPositions.ts` models the three layers that decide this — which top-level nodes the pass enters, which statement kinds it extracts from, and which slots it skips — and its test runs the real pass on the same programs and asserts the two agree. So the residuals listed below for calls do not exist for casts: a cast in one of those positions is AG1017 (there is a line to move it to) or AG1019 (a module-level initializer or a parameter default, where there is not). A cast in a handler body gets AG1020, whose advice is to do the cast before the `handle` block, because the pass never rewrites a handler body and moving the cast to its own line inside it changes nothing. What any pausing Agency call should do in a handler body is still open.
...
Residual shapes the pass does not cover can still desync. There are four of them. Calls nested inside opaque positions, meaning short-circuit right sides, catch expressions, try operands, and with-modified statements. Block bodies nested inside opaque expressions. Mid-chain method calls inside a hoisted access chain: the chain BASE and every argument hoist, so what remains is a later chain segment re-running an earlier method call. Finally, two holes the expressionSlots refactor DISCOVERED in the original pass and preserved for behavior identity. Those are bare method-call statements, where `result.push(transform(item))` does not hoist `transform`, and bare expression statements, such as the parsed form of `print(...) + greet`. Closing those two is a deliberate follow-up with its own fixtures, not a refactor side effect. Those now fail loudly instead of corrupting silently: every frame is stamped with its owner's scope name at CLAIM time, and a mismatched claim throws
