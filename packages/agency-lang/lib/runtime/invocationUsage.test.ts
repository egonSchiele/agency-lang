...
const fullCost = {
  ...
  cacheCreationInputCost: 0.04,
  hostedToolsCost: 0.05,
  totalCost: 0.42,
  currency: "USD",
};
const fullTokens = {
...
describe("normalizeObservation — provider cost", () => {
  ...
  it("maps absent/negative/NaN/±Infinity components to 0 without touching the total", () => {
    const d = normalizeObservation({
      ...
      cost: {
        ...
        cachedInputCost: Number.POSITIVE_INFINITY,
        cacheCreationInputCost: Number.NEGATIVE_INFINITY,
      } as any,
    });
  ...
  it("treats an invalid or non-USD total as unpriced (all cost zero, +1 unknown) but keeps the entry+tokens", () => {
    const nonUsd = normalizeObservation({
      type: "provider",
      kind: "completion",
      reportedModel: "m",
      cost: { ...fullCost, currency: "EUR" } as any,
      tokens: fullTokens as any,
    });
    expect(nonUsd.cost).toEqual({
      inputCost: 0,
      outputCost: 0,
      cachedInputCost: 0,
...
describe("normalizeObservation — tokens & model", () => {
  it("uses the provider totalTokens verbatim (cached-image not double-counted)", () => {
    const d = normalizeObservation({
      ...
      kind: "image",
      reportedModel: "img",
      tokens: { inputTokens: 100, outputTokens: 0, cachedInputTokens: 30, totalTokens: 100 } as any,
    });
    expect(d.tokens.totalTokens).toBe(100);
  });
  it("kind-specific fallback when totalTokens absent", () => {
    const comp = normalizeObservation({
      type: "provider",
      kind: "completion",
      reportedModel: "c",
      tokens: {
    ...
    const img = normalizeObservation({
      type: "provider",
      kind: "image",
      reportedModel: "i",
      tokens: { inputTokens: 10, outputTokens: 2, cachedInputTokens: 5 } as any,
    });
  ...
  it("present-malformed totalTokens falls back and degrades", () => {
    const d = normalizeObservation({
      ...
      kind: "completion",
      reportedModel: "c",
      tokens: { inputTokens: 4, outputTokens: 2, totalTokens: -1 } as any,
    });
  ...
  it("resolves reported → configured → unknown model", () => {
    ...
    expect(normalizeObservation({ type: "provider", kind: "completion" }).entry?.model).toBe(
      "unknown model",
    );
  });
});
...
describe("InvocationUsageMeter", () => {
  it("keeps separate (kind, model) buckets in first-seen order and reconciles when complete", () => {
    ...
    expect(Math.abs(sum - usage.cost.totalCost)).toBeLessThanOrEqual(
      usageReconcileTolerance(usage.cost.totalCost),
    );
  });
  it("snapshot returns deep copies (top-level + nested entry fields)", () => {
    ...
    m.merge(
      normalizeObservation({
        ...
        reportedModel: "opus",
        cost: { totalCost: 0.1, currency: "USD" } as any,
        tokens: { inputTokens: 5, outputTokens: 1, totalTokens: 6 } as any,
      }),
    );
    const a = m.snapshot();
...
describe("normalizeIpcUsageDelta — recover, never drop", () => {
  ...
  it("recovers a missing total as an input+output lower bound (never sums cache) and degrades", () => {
    ...
    const d = normalizeIpcUsageDelta({
      ...
      entry: { kind: "image", model: "img", cost: fullCost, tokens: fullTokens },
    });
    expect(d?.tokens.totalTokens).toBe(100);
    expect(d?.attributionLost).toBe(true);
  });
  ...
  it("returns / rethrows identity", () => {
    expect(
      unwrapServedInvocationOutcome({
        ...
        value: "hi",
        ...snap,
      } as ServedInvocationOutcome<string>),
    ).toBe("hi");
