...
node was given) — see `docs/dev/hosting/statelog.md` — so a trace stands on its own.
...
`docs/dev/evals/eval-labeling.md`. It is over a group (`resolveLabelingGroup` in
...
- `agency label <path…> --checklist <file>` (`lib/cli/eval/label.ts`) — the
  same walk into one group; see `docs/dev/evals/eval-labeling.md`.
