...
export const recommendsCuts = (p: { sourceFileText: string; cleaned: string }): Rubric => ({
  standard:
    "These are the findings of a readability review of the text marked 'The text that was reviewed'. The findings themselves are not the text; do not compare them with the editor's version. Compare the reviewed text with the editor's version and list what the editor removed: sentences, details, or whole sections whose facts are gone from the editor's version. (If the editor's version is empty, the editor removed everything.) A clause the editor reworded or condensed while keeping its fact was not removed; do not list it. Then check the findings: do they say that the removed material should be cut, or that it does not belong? A finding that only rewords something the editor removed does not count. Score in proportion to how much of what the editor removed the findings call to cut. If the editor removed nothing, score 1 when the findings recommend no cuts, and lower the score in proportion to the cuts they do recommend.",
