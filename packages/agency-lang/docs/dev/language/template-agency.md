...
How templates (holes, `fill`, hygiene) work under the hood. The user-facing story is in `docs/site/guide/template-agency.md`. The design history is in the repo-root `docs/superpowers/specs/2026-07-22-code-templates-design.md`, two levels above this package. This doc is for changing the implementation without re-learning it. Shipped in #665.
...
| `lib/types/codeLiteral.ts` | The `CodeLiteral` node (`[| ... |]`) |
...
`findHoles` / `holeNames` / `holeInfos` (`lib/utils/holes.ts`) are filters over `walkNodesArray`, and so are hygiene's `bindersOf` / `freeNamesOf`. That makes **`walkNodes`' descent completeness load-bearing for safety**. If the walker misses a node kind's expression children, free names are under-reported, no test fails, and a filler silently captures a template binder. That is the exact bug hygiene exists to prevent, failing open.
...
The tripwire is now **structural**: the walker-coverage corpus invariants in `lib/utils/expressionSlots.test.ts` (shipped after #665, in the follow-ups PR). Two invariants run over the whole corpus in BOTH parse modes (lowered = the compile pipeline's view; unlowered = the template/hygiene view): slot-table agreement (every `expressionSlots` expr of a walked node is itself walked — a consistency check) and structural reachability (every expression-typed node found by a generic field crawl must be walked, unless its owning field carries a recorded `WALKER_EXCLUDED_FIELDS` ruling or a `KNOWN_WALKER_GAPS` entry). Known gaps are deliberate debt: **walker fixes never land in the PR that discovers them** (walkNodes backs symbol-table scope resolution → codegen), each entry names its follow-up issue (#668), and a staleness guard asserts each entry still shields something — so a walker fix cannot merge without deleting its entry. The hole-position battery in `lib/parsers/hole.test.ts` remains, demoted to *parser* coverage (a hole must PARSE in each position; the walker guarantee is the invariants' job). `typeTestExpression` needs no battery entry: it is a lowering artifact, and templates parse with `lower: false`.
...
The implementation rides on four tarsec ≥0.5.1 features that exist because this feature needed them:

- **The end-scan is grammar, not a lexer.** `codeLiteralBodyChunk` is an `or(...)` of the REAL region parsers — `_stringParser` (escapes and `${...}` interpolations, nested strings included), the two comment parsers, `regexLiteralParser` (`re/a|]b/` must not end the literal; `re/[|]/` must not read as nesting), a committed nested-`[|` probe, and `anyChar` — under `matchedText(repeatTill(chunk, str("|]")))`. The bulk alternative (`many1TillOneOf` of the trigger characters) swallows inert runs in one step; if you add a new region kind to the language (a new literal form with arbitrary text), add its parser as a chunk AND its first character to the trigger set, or `|]` inside it will terminate literals early. Zero escaping rules is a *consequence*: `|]` in code position is not legal Agency, and every region where it is legal text is consumed by that region's own parser. The scan runs under a rightmost save/restore so its probe failures cannot leak onto the enclosing record (restore does not touch the committed-failure slot).
- **Nested parses run in `runNested`** — own input string, rightmost record, and memo caches, restored on exit — with `basePosition` set so body spans AND error messages come out in enclosing-file coordinates natively. The one coordinate subtlety left on our side: `basePosition` is passed in USER coordinates (absolute line minus `currentTemplateOffset`) and the template offset is zeroed around the nested parses, because `withLoc` inside them would otherwise subtract the offset a second time. Leading/trailing trivia is stripped symmetrically for the expr/statements attempts — whitespace AND `BLANK_LINE_SENTINEL` (a sentinel is not JS whitespace; leaving one would flip an expr body to statements) — and `advancePositionOver` counts sentinels as line breaks, because they *replaced* newlines.
- **The literal is `committed(str("[|"), rest)`.** Once `[|` is seen, no fallback parser reinterprets the text: failures inside are final and win error reporting outright (`getErrorMessage` prefers the committed failure). One integration wart to know: tarsec's `parseError` THROWS its own generic message (`TarsecError`) even past a committed failure — but the committed failure survives in `getParseState().committedFailure`, so both of `parseAgency`'s failure exits consult it, and the structured `errorData` position comes from the committed failure's `rest` (not the rightmost record — the squiggle must land where the message points).
- **Compile and runtime share one parse.** Codegen embeds `printCodeLiteralBody(node)` — the identical text `fmt` shows between the brackets (one shared body printer) — as `__codeLiteral(body, kind)`. The runtime reconstructs through the SAME exported `parseCodeLiteralBody` and asserts the recorded kind, so the two stages cannot diverge by drift. Reconstruction is cached per `(kind, source)` (literal bodies are compile-time constants; a literal in a loop parses once) and returns `deepCopy` clones, because `fill` mutates Code values downstream. `codeLiteral.ts` carries a **load-bearing side-effect import of `parser.js`**: program-kind bodies parse through the grammar entry point that `parser.ts` registers into `parsers.ts` at module init (the cycle-avoiding injection), and nothing else in the runtime's import graph reaches it — remove that import and a program-kind literal in a program with no `std::agency` import fails at runtime with an "impossible" internal error (pinned by the `literalNoImports` fixture).

Host-side, the node is a **leaf** — quoted names belong to the generated program — enforced by five separate levers, each with a forcing function: no `walkNodes` case (per-type enumeration = leaf for free); `codeLiteral.nodes` deliberately NOT in `bodySlots` (the generic tail descent; the one lever that is an *absence*, pinned by a comment at the spot where someone would helpfully add it plus a test); `NO_EXPRESSION_SLOTS` registration (the completeness test fails by name without it); the `WALKER_EXCLUDED_FIELDS["codeLiteral.nodes"]` tripwire ruling; and `identifierSlots.ts`, which is exhaustively typed over `AgencyNode` and demands its entry at compile time. Host hygiene must NOT see quoted names (a host `tmp` and a quoted `tmp` are unrelated); quoted-code hygiene runs later, at fill time, on the runtime value.

Typechecking: a literal synthesizes the **structural equivalent of stdlib `Code`** — built once by parsing the type TEXT through the type-alias production (the same grammar Code's own declaration uses), because the checker's encoding of a record type is a grammar decision (`kind?:` → null-union, quoted literals → `stringLiteralType`) and hand-building the tree re-makes those decisions by hand, which is exactly what broke the first attempt. The singleton is deep-frozen (a future in-place pass should fail immediately, not spookily) and a drift test compares it structurally against the `Code` alias parsed from the real `stdlib/agency.agency`. The widening story lives in `widenType` itself (`isCodeShape` in `assignability.ts`): a record whose `type` field is the `"agencyProgram"` literal survives widening wherever it appears — direct binding, array elements, branch reassignment, inferred returns. An earlier declare-site syntactic guard covered only the direct case; if you see a `Code` value the checker claims is not one, with the literal nowhere near the error, suspect a new widening path that bypasses `widenType`.

Decisions pinned by tests so they cannot drift silently: empty body `[| |]` is an empty `statements` fragment (matching `parseStatements("")` — the literal and the runtime parser must never disagree about the same text); `|]` in interpolation *code* position is inert (it belongs to the generated program's string); nesting is a directive error ("build the inner piece as its own value and graft it"); `${...}` in a body passes through to the generated program (no `$( )` splices — holes are the only parameterization, and note that FIXTURE host strings need `\${x}` escapes while literal bodies do not, which trips everyone once).

`fmt` **reformats literal bodies** — a body is held to the same standard a file is — printed via the shared body printer, inline for one-line expr bodies, one indent level deeper otherwise. This is guarded by the round-trip gate (`agencyGenerator.roundtrip.test.ts`): whole-corpus unlowered print→re-parse structural identity plus print idempotence, with byte-exact goldens. The gate's comparison contract matters when you touch the formatter: code nodes compare in order; imports and comments compare as content-preserving multisets (sorting and hoisting are policy); docstring text compares whitespace-collapsed; vertical-whitespace stability is the idempotence invariant's job. Two formatter policies the gate enforces indirectly and tests pin directly: runs of blank lines collapse to one (file-wide), and a parenthesized access-chain base keeps its parens unless it is in the `SAFE_BARE_BASES` allowlist — fail-closed, because the deny-list version corrupted `(a ?? "").toUpperCase()` and printed `(new Foo()).bump()` unparseably. Intentionally-unparseable `expectedCompileError` fixtures are excluded from the gate corpus by name, with reasons.
...
- **AG8002** (`checkTemplateHoles` in `lib/typeChecker/templateHoles.ts`, wired into `typeCheck`) fires for an expr hole with neither an inline annotation nor a position-supplied type — v1 recognizes the untyped-assignment position only. The synthesizer types a hole as its annotation, else `any`.
...
| `lib/parsers/codeLiteral.test.ts` | Kind inference, end-scan inertness (strings/comments/regex/interpolations, content-asserted), nested/unclosed directives, hand-computed location mapping in both offset modes, empty-body ruling |
| `lib/typeChecker/codeLiteral.test.ts` | Literal↔`Code` structural compatibility (incl. wrapped: array element, branch reassignment), quoted-leaf non-diagnosis with a sanity anchor, definite returns, the stdlib-`Code` drift guard |
| `lib/backends/agencyGenerator.roundtrip.test.ts` | The formatter gate: corpus round-trip + idempotence, literal goldens, pinned repros for the four pre-existing generator bugs and the two review-round paren cases, blank-line policy |
| `lib/utils/bodySlots.test.ts` (literal case) | `bodySlots(codeLiteral) === []` and the walker never yields quoted nodes |
| `tests/agency/templates/` | Execution fixtures: AG8001 + AG8002 (`expectedCompileError`), checkpoint survival (file and literal built), end-to-end injection, parent-handler governance, compose-then-parameterize (file and literal), `holesOf` origin, `${}` passthrough, malformed-literal compile error, no-imports registration, a return-type-less node declaration inside a literal, a statement-position splice in a node body, and `toSource` on a deep-frozen `static const` Code value |
...
- Tarsec keeps parse state per-parse (input string, rightmost record, memo caches, the committed-failure slot); the ONLY supported way to parse inside a parse is `runNested`. Calling `setInputStr` or a raw grammar entry mid-parse corrupts the enclosing parse's positions — the original symptom was a `RangeError` inside error *formatting*.
