...
> **Why seed from all files, not the first.** `SymbolTable.build` (`lib/symbolTable.ts:129`, signature `entrypoint: string | string[]`) crawls imports transitively from its entrypoint(s) only. Each file is later typechecked via `buildCompilationUnit(..., symbolTable, fromFile)`, which resolves that file's imports with `symbolTable.resolveImport(stmt, fromFile)` — a lookup into `this.files[resolvedImportPath]`. If a file's imports were never reachable from the seed, that lookup returns `undefined` and the import silently resolves to nothing, and `symbolTable.getFile(fromFile)` also returns `undefined` (dropping the file's interrupt-effect metadata). For a directory whose files import each other divergently (the primary `agency tc src/` use case), a single-file seed produces false-positive type errors and a spurious exit 1. Passing all file paths is both correct and free — the loop parses those files anyway.
