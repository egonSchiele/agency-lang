...
arm that reads the directory from the interrupt's data field
(\`"std::write" if (data.data.dir == ".") => approve()\` when the handler
parameter is named data), not an if inside
the arm. Every reject is \`reject()\` with no message argument (\`return reject()\`
is fine). The Result that foo returns is
