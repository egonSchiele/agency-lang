...
- **Effects may be bare or namespaced.** `std::read` and a bare `deploy` are both valid effect labels at `raise`/`interrupt` sites and inside `<...>`. Inside a `<...>` set, a `::`-bearing item is a literal effect label, while a bare identifier is resolved at typecheck time — spread if it names a known `effectSet`, otherwise treated as a literal bare effect. (`namespaceIdentifier` at parsers.ts:2522 still requires `::`; `effectIdentifier` adds a bare fallback that excludes the `interrupt`/`raise` keywords.)
