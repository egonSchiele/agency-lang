...
- After the model turn, discover the `.agency` files the agent created or modified **this turn**. **Preferred mechanism: `std::git`** — the working directory is typically a git repo, so `gitStatus` (via `std::git`) lists the changed paths, filtered to `*.agency`.
- **Turn-scoping is mandatory, not optional.** `git status` reports *all* currently-dirty files, including `.agency` files that were already uncommitted before the task started. Reviewing those would re-introduce the exact bug this sub-project fixes: a pre-existing dirty `.agency` file with an error would flag `feedbackHasErrors`, thrash the agent on a file it never touched, and re-shadow `verify`. So capture a **baseline** set of dirty `.agency` paths at task start (before the agent acts) and review only `current − baseline` — the files that became dirty *during* the run. (Non-git fallback: `glob` + start-of-task snapshot, same set-difference.) A known benign gap: a file that was *already* dirty and the agent *also* edits is skipped by the set-difference; that under-reviews rather than false-flags, and `verify` still runs.
