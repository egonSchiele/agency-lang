...
live on a hosted statelog server. The server runs them on a cron, and the CLI
is only a management client. This note covers the seams and the decisions that are easy to get
...
`edit` take the **server schedule id** shown by `list`, not a local name. The
same positional argument means a name on the local path and an id on the
...
- `lib/cli/schedule/remote.ts` — pure resolvers plus thin recipes.
  ...
  declarative request values. They throw on every invalid flag combination
...
through the client. A hand-rolled `fetch` with a `response.ok` check will
...
returns an exhaustive `RunDeployOutcome`: `"deployed" | "aborted" | "preview"`.
A deploy error exits the process instead of returning. A resolved promise is
NOT proof of an upload, because declining the no-exports confirmation resolves
with `"aborted"`, so schedule creation is gated on the `"deployed"` outcome
...
name, and args are immutable, so changing them means delete-and-re-add. The CLI
