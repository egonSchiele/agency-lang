...
The handler receives a parameter named `data`. `data` has an `effect` field for the effect name and a `data` field for the interrupt's additional data. Note that you can set guards on match arms. See the `"std::write"` arm for an example.
