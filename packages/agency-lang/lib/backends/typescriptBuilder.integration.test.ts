...
`;
  ...
  // The special case cannot hijack a user variable, because a user cannot
  // have one by this name: the parser refuses every `__` name.
  it("has no user variable named __matchval_<n> to confuse with the temp", () => {
    const source = `node main() {
  let __matchval_9 = 5
  return __matchval_9
}
`;
    expect(() => generateWithBuilder(source, "matchvalUserLocal.agency")).toThrow(
