...
That only holds for Agency code. A plain JS/TS package passes through untouched when imported (`docs/dev/language/pkg-imports.md`), and JavaScript raises nothing, so a generator that reaches one is neither checked before it runs nor stopped while running.
