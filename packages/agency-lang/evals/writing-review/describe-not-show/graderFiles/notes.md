...
- The whole text describes syntax in prose, with code fragments inside parentheses inside sentences. One code block showing the handler, with the guarded arm in it, would say all of it at once. Lead with the example; then the checklist can point at it ("as shown above").
