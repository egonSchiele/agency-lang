...
function compileForServe(file: string, options: { quiet?: boolean } = {}): CompileResult {
  ...
  const { moduleId, exportedNodeNames, interruptEffectsByName, errors } = collectServeMetadata({
    filePath: file,
    config,
  });
...
async function loadAndDiscover(
  ...
  const toolRegistry = (moduleExports.__toolRegistry as Record<string, any>) ?? {};
...
async function generateStandalone(
  ...
  try {
    await esbuild.build({
      ...
      format: "esm",
      outfile,
      external: ["node-llama-cpp", "node-llama-cpp/*", "@node-llama-cpp/*", "@reflink/*"],
      banner: {
        js:
    ...
    if (bundleSucceeded && path.basename(compiledAbsPath) === `${baseName}.js`) {
...
function buildEntrypoint(
  ...
  switch (mode) {
    case "http":
      return buildHttpEntrypoint(compiledAbsPath, compileResult, options as StandaloneHttpOptions);
    case "mcp":
