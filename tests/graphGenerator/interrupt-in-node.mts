...
if (isInterrupt(__self.response)) {
  ...
  return { messages: __self.messages_0 , data: __self.response };
      ...
      if (__step <= 4) {
        return { messages: __self.messages_0, data: __stack.locals.response}
        __stack.step++;
      }
...
graph.node("sayHi", async (state): Promise<any> => {
        ...
        return goToNode("foo2",
  {
    ...
    data: [__stack.args.name, __stack.locals.age]
...
export async function foo2(name, age, { messages, callbacks } = {}): Promise<__State<string>> {
...
export async function sayHi(name, { messages, callbacks } = {}): Promise<__State<any>> {
