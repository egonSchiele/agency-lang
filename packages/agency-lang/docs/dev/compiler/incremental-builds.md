...
| `compilerStamp` | Content hash of the compiled modules the compile pipeline imports: `computeCompilerStamp` follows relative imports from `dist/lib/compiler/buildSession.js` and hashes every module it reaches except those under `runtime/` (generated text does not depend on runtime internals; runtime modules are still walked, so what they import is hashed). Code the pipeline does not import, such as the logs viewer, the TUI, or the LSP, cannot change compiled output, so editing it rebuilds nothing. Before this, the stamp hashed all of `dist/lib`, and a one-line logs-viewer edit cost about 20 seconds of recompiling and doc regeneration. The compiled agents under `dist/lib/agents` are never imported; they are the agency compiler's own output, and hashing them would make every build invalidate the next. A string in compiled code that looks like an import can only add a file to the hash. If the entry file is missing (renamed or moved), a walk would reach nothing and the stamp would never change, so `computeCompilerStamp` hashes every module except `runtime/` and `agents/` instead: slow, never stale. `compilerStampEntries.test.ts` fails when an entry's source file is renamed, so that fallback is never the normal path in `dist`. Under vitest the entry is always missing and both writer and checker take the fallback. Content, not mtimes: `tsc-alias` rewrites the whole outDir every build. |
