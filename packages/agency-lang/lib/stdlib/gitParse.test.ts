...
describe("parseLog", () => {
  it("parses commits with multi-line bodies; tolerates git's inter-record newline", () => {
    ...
    const out =
      ...
      RS +
      "\n" +
      record(["sha2", "Bob", "bob@x.com", "2026-01-02T00:00:00Z", "subj two", ""]) +
      RS +
      "\n";
...
describe("parseBranchList", () => {
  it("marks current, captures upstream + sha; asserts full array", () => {
    const out =
      ["main", "*", "origin/main", "aaa"].join(FS) +
      RS +
      "\n" +
      ["feature/x", " ", "", "bbb"].join(FS) +
      RS +
      "\n";
...
describe("parseBlame", () => {
  it("pairs each porcelain header (real hex sha) with its content line", () => {
    const out =
      [
        "1a2b3c4d5e6f7a8b 1 1 1",
        "author Amy",
        "\tconst x = 1",
        "9f8e7d6c5b4a3210 2 2 1",
        "author Bob",
        "\tconst y = 2",
      ].join("\n") + "\n";
  ...
  it("derives status + counts across modified, deleted, renamed, text-new, and binary files", () => {
    const patch =
      [
        ...
        "@@ -10 +11,2 @@",
        " ctx2",
        "+another", // second hunk (multi-hunk)
