...
export function parseAgency(
  ...
  try {
    ...
    } else if (error instanceof LoweringError) {
      // Compile-time error from expression-position `match` lowering (all-paths
      // yield, bare return, concurrency-boundary return, module-level match
      // expression, `is`-form in expression position). Surface as a normal
      // failed parse, same as PatternLoweringError above.
      return {
        success: false,
        message: error.message,
        rest: input,
