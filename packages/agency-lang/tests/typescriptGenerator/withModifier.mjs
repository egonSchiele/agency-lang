...
let __functionCompleted = false;
  ...
  const runner = new Runner(__ctx, __stack, { state: __stack, moduleId: "withModifier.agency", scopeName: "foo", threads: __threads });
...
let __forked;
let __functionCompleted = false;
  const runner = new Runner(__ctx, __stack, { nodeContext: true, state: __stack, moduleId: "withModifier.agency", scopeName: "main", threads: __threads });
  try {
    await runner.hook(0, async () => {
await callHook({
        name: "onNodeStart",
