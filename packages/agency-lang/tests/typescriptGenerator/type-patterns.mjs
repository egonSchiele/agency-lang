...
return;
  ...
  {
    condition: async () => await __typeTest(__validateType(__stack.args.value, Person)) && !__eq(__stack.args.value, null) && __coarseTypeTest(__stack.args.value, "object"),
    body: async (runner) => {
...
return;
  ...
  {
    condition: async () => await __typeTest(__validateType(__stack.args.value, z.object({ "tag": z.string() }))),
    body: async (runner) => {
