...
- `docs/dev/std-agency-test.md` — std::agency test()/testFile() + the eval AgencyTestGrader: the closure validator's pure-Agency invariant and mirror TOCTOU boundary, one-vote scripted answers (and the outer-valueless-approve merge wart), gate-before-read testFile, files/-vs-holdout/ discovery, the synthesized grading module's persisted revision, the phased wrapper policy, and the file report envelope
