...
- **subprocess IPC** (`std::agency` `run` / `runFile`): the child runs a *separately-compiled* program via `runNode` in a fresh process with its own provider registry, so it must re-register. Two channels make that automatic: the `AGENCY_PROVIDER_MODULES` env var is inherited via `buildForkOptions`, and the parent's config-declared `providerModules` are forwarded through the existing `configOverrides` channel (`_run` absolutizes them against the parent cwd; `applyRuntimeConfigOverridesToContextArgs` merges them into the child's `RuntimeContext` args). Relative paths are absolutized because `run(cwd:)` can give the child a different cwd.
