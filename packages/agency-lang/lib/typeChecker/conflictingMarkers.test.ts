...
const conflictError = (source: string) =>
  typeCheckSource(source).errors.find((e) => (e as { code?: string }).code === "AG7006");
...
describe("conflicting destructive/idempotent markers", () => {
  ...
  it("errors regardless of marker order", () => {
    expect(conflictError("idempotent destructive def f() { return 1 }")).toBeDefined();
  });

  it("does not error on a plain destructive def", () => {
    expect(conflictError("destructive def rm(p: string) { return 1 }")).toBeUndefined();
  });

  it("does not error on a plain idempotent def", () => {
    expect(conflictError("idempotent def f(): number { return 1 }")).toBeUndefined();
  });
