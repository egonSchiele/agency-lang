...
- **`return`-to-match may not cross a concurrency boundary.** A `return` inside a `parallel`/`seq`/`fork`/`race`/`thread` block within an arm is a compile error — those branches are separate execution contexts the `_matchExit` unwind cannot cross.
