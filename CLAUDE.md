...
- `docs/dev/statelog-to-dataset.md` — Promoting a statelog trace into a label dataset (viewer `l` + `agency label ingest <statelog> --trace`): the pure resolve→select→project pipeline (loaders can't prompt), the strict output precedence and truncation rule, the `statelog` occurrence origin + locator identity and the non-destructive manifest v2→v3 upgrade, the shared datasetWriter/labelingHost effect owners and sequential lock ordering, the viewer hook + focusItem, and the `print` statelog event that feeds it
