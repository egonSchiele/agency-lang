...
describe("HumanGrader", () => {
  it("normalizes a scalar rating against the scale (min maps to 0, max to 1)", async () => {
    ...
    const grade = await new HumanGrader({ scale: { min: 1, max: 10 }, read }).run(
      graderInput("some code"),
    );
  ...
  it("passes the prompt, scale, and stringified artifact to the reader", async () => {
    let seen:
      { prompt: string; artifact: string; scale?: { min: number; max: number } } | undefined;
    const read: HumanRead = async (req) => {
      seen = req;
      return { rating: 1 };
    };
    await new HumanGrader({
      name: "quality",
      prompt: "Rate it",
      scale: { min: 0, max: 2 },
      read,
  ...
  it("fails fast on a non-finite or out-of-range rating", async () => {
    ...
    await expect(
      new HumanGrader({ scale: { min: 1, max: 10 }, read: outOfRange }).run(graderInput("x")),
    ).rejects.toThrow(/expected a rating in \[1, 10\]/);
    const noRating: HumanRead = async () => ({ note: "only a note" });
    await expect(
      new HumanGrader({ scale: { min: 1, max: 10 }, read: noRating }).run(graderInput("x")),
  ...
  it("fails fast when a binary grader gets no verdict", async () => {
    ...
    await expect(new HumanGrader({ read: noVerdict }).run(graderInput("x"))).rejects.toThrow(
      /expected a pass\/fail/,
    );
  });
});
