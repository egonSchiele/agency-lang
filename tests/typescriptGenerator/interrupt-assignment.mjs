...
graph.node("main", async (state) => {
    ...
    await callHook({ callbacks: __ctx.callbacks, name: "onNodeStart", data: { nodeName: "main" } });
      ...
      if (__step <= 1) {
        if (__ctx.stateStack.interruptData?.interruptResponse?.type === "resolve") {
  __stack.locals.name = __ctx.stateStack.interruptData.interruptResponse.value;
...
__self.greeting = _greeting({
      ...
      if (__step <= 4) {
        return { messages: __threads, data: __stack.locals.greeting}
        __stack.step++;
      }
