...
describe("layout", () => {
  ...
  it("column direction stacks children vertically", () => {
    const el = column(box({ key: "a", height: 5 }), box({ key: "b", height: 5 }));
    const result = layout(el, 80, 24);
  ...
  it("row direction places children horizontally", () => {
    const el = row(box({ key: "a", width: 20 }), box({ key: "b", width: 30 }));
    const result = layout(el, 80, 24);
    const a = result.children!.find((c) => c.key === "a")!;
    const b = result.children!.find((c) => c.key === "b")!;
  ...
  it("flex distributes remaining space", () => {
    const el = row(box({ key: "a", width: 20 }), box({ key: "b", flex: 1 }));
    const result = layout(el, 80, 24);
    const b = result.children!.find((c) => c.key === "b")!;
  ...
  it("multiple flex children split space proportionally", () => {
    const el = row(box({ key: "a", flex: 1 }), box({ key: "b", flex: 2 }));
  ...
  it("border reduces inner space by 2 in each dimension", () => {
    const el = box({ width: 20, height: 10, border: true }, box({ key: "inner", flex: 1 }));
    const result = layout(el, 80, 24);
  ...
  it("nested layout", () => {
    const el = column(
      ...
      row({ flex: 1 }, box({ key: "left", width: "50%" }), box({ key: "right", flex: 1 })),
    );
  ...
  it("padding reduces inner space", () => {
    const el = box({ width: 20, height: 10, padding: 2 }, box({ key: "inner", flex: 1 }));
    const result = layout(el, 80, 24);
    const inner = result.children!.find((c) => c.key === "inner")!;
  ...
  it("margin offsets element position", () => {
    const el = column(box({ key: "a", height: 5, margin: { top: 2, left: 3 } }));
    const result = layout(el, 80, 24);
    const a = result.children!.find((c) => c.key === "a")!;
  ...
  it("justifyContent flex-end pushes children to end", () => {
    const el = column({ justifyContent: "flex-end" }, box({ key: "a", height: 5 }));
    const result = layout(el, 80, 24);
    const a = result.children!.find((c) => c.key === "a")!;
  ...
  it("justifyContent center centers children", () => {
    const el = row({ justifyContent: "center" }, box({ key: "a", width: 20 }));
    const result = layout(el, 80, 24);
    const a = result.children!.find((c) => c.key === "a")!;
  ...
  it("alignItems center centers children on cross axis", () => {
    const el = row({ alignItems: "center" }, box({ key: "a", width: 20, height: 10 }));
    const result = layout(el, 80, 24);
    const a = result.children!.find((c) => c.key === "a")!;
  ...
  it("alignItems flex-end positions children at cross axis end", () => {
    const el = row({ alignItems: "flex-end" }, box({ key: "a", width: 20, height: 10 }));
    const result = layout(el, 80, 24);
    const a = result.children!.find((c) => c.key === "a")!;
  ...
  it("alignItems stretch fills cross axis (default)", () => {
    const el = row(box({ key: "a", width: 20 }));
    const result = layout(el, 80, 24);
    const a = result.children!.find((c) => c.key === "a")!;
