...
- `docs/dev/env-gating.md` — Why `env()` raises `std::env`, the rule for deciding how a call site answers it (value escapes → propagate; presence check or already-gated operation → `with approve`), why `with approve` works in a static initializer but does not discharge the effect from `raises`/AG3009, the one place `--reject` cannot override it, why AG7003 cannot catch this class, and why the test has to be a round trip
