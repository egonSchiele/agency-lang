...
1. All new agency standard library modules go into the `stdlib/` directory and require an agency file. A TypeScript file may also be necessary for some of the backing functionality, but is not required. A module may live in a subdirectory, which becomes part of its import path: `stdlib/aws/s3.agency` is imported as `std::aws/s3`. There is no module registry to update. The compiler and the LSP discover modules by scanning `stdlib/`.
...
6. If your module reads, writes, lists, or probes a file, do it through `lib/stdlib/contained.ts`, never through `fs` directly. Ask where the path came from. If the user or the agent chose it, the directory they named is the root: `readText(root(dir), filename)`. If it is a whole path such as the target of a delete, `wholePath(p)` splits it into a real parent and a final name. The module refuses any symlink below the root, and a lint rule fails the build if a stdlib file imports `fs` without an entry in `FS_IMPORTERS` in `eslint.config.js`. The reasoning and the full API are in `docs/dev/stdlib/contained-files.md`.

7. We automatically generate documentation for any code in the agency standard library. To generate the documentation, run `make` or `make doc`. Either one runs `agency doc stdlib -o docs/site/stdlib/`. If you are adding a new file to the agency standard library, you also need to link its page in the `/stdlib/` sidebar in `docs/site/.vitepress/config.mts`. Every stdlib module is listed there, in alphabetical order. A nightly scheduled agent, `.github/workflows/stdlib-docs-links.yml`, checks for stdlib pages nobody linked. For the doc-comment conventions the generator understands, see `docs/site/cli/doc.md`.
...
That is all there is to it. From the codegen's perspective `_doThing` is an ordinary imported function. The convention is to prefix the JS export with a single underscore so it reads as a stdlib JS helper meant only for its `.agency` wrapper. The codegen treats the name like any other import.
...
```ts
import { RuntimeContext, StateStack, ThreadStore, runInTestContext } from "agency-lang/runtime";
...
Bad:
```
export idempotent def input(prompt: string): string {
  """
...
Also read the [general writing tips](../contributing/general-writing-tips.md) and [anti-patterns](../contributing/anti-patterns.md) guides for more information on writing good documentation and code.