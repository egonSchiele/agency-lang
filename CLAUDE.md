...
- `docs/dev/data-connectors.md` — Writing a `std::data` connector: the shared core (`connectorFetch`'s internal approve as a vote not a bypass, `connectorError`, `clampLimit`, `dateStrToEpochMs`), the 8-part anatomy modeled on `stdlib/data/social/bluesky.agency`, the conventions (epoch-ms times, union-typed enums, clamped limits, `idempotent` reads, payload carries what handlers judge), the fetchMocks testing template, and how to update or port a connector
