...
| `evaluate(ws, source, files, inputs)` | Grade `files` on `inputs` inside an existing workspace `ws`. Runs are cached by `(ws.key, inputId)`, so re-evaluating a candidate you already scored on an input is free. A candidate that crashes the agent on an input is scored, not fatal: `runInputViaEval` notes the error through the reporter and returns the run directory, whose `run` row says the run ended in error, and `gradeRun` scores that 0 without calling any grader. |
