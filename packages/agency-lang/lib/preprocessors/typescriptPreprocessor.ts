...
export class TypescriptPreprocessor {
  ...
  /**
   * Prune auto-imported `std::index` prelude symbols that this file shadows
   * with its own top-level `def` / `node` / global-variable declaration.
   *
   * The prelude (`import { ... } from "std::index"`) is injected invisibly
   * into every file. In JS, an import and a same-name top-level declaration
   * are a duplicate binding — so without this, a user's `def map` or global
   ...
   * the local `const map` initializes) or "Assignment to constant variable"
   * for globals (the import is a `const`).
   *
   ...
   * declaration becomes the sole module-level binding. Only `std::index`
   * imports are touched; the builder derives both the emitted import and its
   * `__registerTool(...)` call from these specifiers, so pruning removes both.
   */
  protected prunePreludeShadows(): void {
    ...
    this.program.nodes = this.program.nodes.filter(
      (node) =>
        node.type !== "importStatement" || node.importedNames.length > 0,
    );
  }
