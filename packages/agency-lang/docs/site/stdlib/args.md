...
Agency's own flags go before the filename. Position decides even when the name
collides: `agency run greet.agency --max-cost 5` sends `--max-cost` to your
program rather than capping the run, and agency prints a warning saying so.
