...
describe("Runner", () => {
  ...
  describe("hook()", () => {
    // Stub callback that looks like an AgencyFunction (callHook uses
    // `isAgencyFunction` to decide whether to expect Interrupt[] back).
    // Tests register one of these on ctx.topLevelCallbacks to exercise
    // the interrupt-propagation path inside Runner.hook.
    const fakeAgencyCb = (returns: any[] | undefined) => ({
      __agencyFunction: true,
      invoke: async () => returns,
    });

    it("happy path: no callbacks registered → advances counter", async () => {
      const frame = makeFrame();
      const ctx = makeMockCtx();
      const runner = new Runner(ctx, frame, { stack: ctx.stateStack });

      await runner.hook(0, "onFunctionStart", { functionName: "foo" });

      expect(runner.halted).toBe(false);
      expect(frame.step).toBe(1);
    });

    it("resume skip: counter past id → no-op (callHook not called)", async () => {
      const frame = makeFrame();
      frame.step = 5;
      const ctx = makeMockCtx();
      // If callHook were invoked the interrupt would surface — proves
      // the skip short-circuited before reaching it.
      ctx.topLevelCallbacks = [
        { name: "onFunctionStart", fn: fakeAgencyCb([{ type: "interrupt", interruptId: "i" }]) },
      ];
      const runner = new Runner(ctx, frame, { stack: ctx.stateStack });
    ...
    it("halt on interrupts: halts with the interrupt batch and leaves counter unchanged", async () => {
      ...
      ctx.topLevelCallbacks = [
        { name: "onFunctionStart", fn: fakeAgencyCb([intr]) },
      ];
      const runner = new Runner(ctx, frame, { stack: ctx.stateStack });
    ...
    it("re-fires on resume: callback now returns undefined → counter advances", async () => {
      const frame = makeFrame();
      const ctx = makeMockCtx();
      let calls = 0;
      const cb = {
        ...
        invoke: async () => {
          calls++;
          return calls === 1
            ? [{ type: "interrupt", interruptId: "i-1" }]
            : undefined;
        },
      };
      ctx.topLevelCallbacks = [{ name: "onFunctionStart", fn: cb }];
      const runner1 = new Runner(ctx, frame, { stack: ctx.stateStack });

      await runner1.hook(0, "onFunctionStart", { functionName: "foo" });
      expect(runner1.halted).toBe(true);
      expect(frame.step).toBe(0);

      // Simulate resume: fresh runner over the same frame.
      const runner2 = new Runner(ctx, frame, { stack: ctx.stateStack });
      await runner2.hook(0, "onFunctionStart", { functionName: "foo" });

      expect(runner2.halted).toBe(false);
      expect(calls).toBe(2);
      expect(frame.step).toBe(1);
    });
  });
