...
describe("StateStack branches serialization", () => {
  it("toJSON serializes branches recursively", () => {
    ...
    const parentFrame = makeFrame({
      ...
      step: 3,
      branches: {
        0: { stack: childStack, interrupt_id: "int-1", interruptData: { msg: "hello" } },
      },
    });
    ...
    expect(serializedFrame.branches["0"].interrupt_id).toBe("int-1");
    expect(serializedFrame.branches["0"].interruptData).toEqual({ msg: "hello" });
  ...
  it("fromJSON deserializes branches into live StateStack instances", () => {
    ...
    expect(frame.branches![0].interrupt_id).toBe("int-1");
  ...
  it("round-trips nested branches (parent → child → grandchild)", () => {
    ...
    const parentFrame = makeFrame({
      ...
      step: 3,
      branches: {
        0: { stack: childStack, interrupt_id: "top-interrupt" },
      },
    });
    const parentStack = new StateStack([parentFrame], "serialize");
    ...
    expect(childBranch.interrupt_id).toBe("top-interrupt");
    ...
    expect(grandchildBranch.stack.stack[0].args).toEqual({ level: "grandchild" });
  ...
  it("toJSON omits interrupt_id and interruptData when not set", () => {
    ...
    expect(branch.interrupt_id).toBeUndefined();
