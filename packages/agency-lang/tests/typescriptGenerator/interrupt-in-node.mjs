...
__registerCallbacksInit("interrupt-in-node.agency", __registerTopLevelCallbacks);
...
if (__response) {
  ...
  if (!isApproved(__handlerResult)) {
    ...
    const __checkpointId = getRuntimeContext().ctx.checkpoints.create(__stateStack(), __ctx, { moduleId: "interrupt-in-node.agency", scopeName: "greet", stepPath: "1" });
...
await callHook({
    ...
    return {
      messages: __threads(),
      data: runtimeFailure(__error, { functionName: "foo2" })
    };
  }
})
...
if (__process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    ...
    const __result = await runCliEntry({
      nodes: {
        foo2: () => foo2(undefined, undefined, initialState),
