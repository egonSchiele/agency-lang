...
- `docs/dev/llm/local-speech.md` — `agency local serve --speech`: the speech server script and why Agency ships its own, the rules module CI can test, the model families and what each takes, sentence splitting, the mlx-audio version pin, how an audio reply is logged, Orpheus with its companion decoder and the three mlx-audio patches it needs, the `speakLocal` function and its effect with how long text is sent in pieces, and why Agency never runs `mlx_audio.server` — read that section before adding any endpoint here.
