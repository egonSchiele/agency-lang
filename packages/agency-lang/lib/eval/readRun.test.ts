...
describe("readEvalRun", () => {
    ...
    writeSummary([
      {
        inputId: "task-1",
        status: "success",
        evalRecordPath: recordPath("task-1"),
        statelogPath: "",
        workdirPath: "",
      },
    ]);
    ...
    writeSummary([
      {
        ...
        status: "success",
        evalRecordPath: recordPath("missing-record"),
        statelogPath: "",
        workdirPath: "",
      },
    ]);
    ...
    writeSummary([
      {
        inputId: "failed",
        status: "error",
        evalRecordPath: recordPath("failed"),
        statelogPath: "",
        workdirPath: "",
        errorMessage: "summary boom",
      },
    ]);
  ...
  function writeInput(inputId: string, input: unknown): void {
    ...
    fs.writeFileSync(
      path.join(runDir, "inputs", inputId, "input.json"),
...
describe("readEvalRun layouts", () => {
  function makeLayoutRun(layout: "legacy" | "current"): string {
    ...
    fs.writeFileSync(
      path.join(layoutRunDir, "summary.json"),
      JSON.stringify({
        runId: "r",
        runDir: layoutRunDir,
        agentLabel: "a:main",
