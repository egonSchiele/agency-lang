...
Oh by the way, interrupts work just the same in tool calls too. Every function in Agency is also automatically a tool. That means an agent can use `deleteEmail` as a tool, and before it deletes any email, it will check with the user, following the rules of handlers.