...
describe("classifyLlmError", () => {
  ...
  it("classifies HTTP errors by status", () => {
    ...
    expect(
      classifyLlmError(err, { status: 429, retryAfterMs: 5000, message: "http" }),
    ).toMatchObject({
      kind: "retryable",
      reason: "rateLimit",
      retryAfterMs: 5000,
    });
    expect(classifyLlmError(err, { status: 529, message: "http" })).toMatchObject({
      kind: "retryable",
      reason: "overloaded",
    });
    expect(classifyLlmError(err, { status: 503, message: "http" })).toMatchObject({
      kind: "retryable",
      reason: "serverError",
    });
  ...
  it("typed terminal kinds classify as terminal", () => {
    expect(classifyLlmError(new Error("x"), { kind: "contentPolicy", message: "x" }).kind).toBe(
      "terminal",
    );
    expect(classifyLlmError(new Error("x"), { kind: "contextWindow", message: "x" }).kind).toBe(
      "terminal",
    );
  });
  ...
  it("the client's own requestTimeout is retryable (transport)", () => {
    expect(
      classifyLlmError(new Error("x"), { kind: "requestTimeout", message: "x" }),
    ).toMatchObject({
      kind: "retryable",
  ...
  it("message-matches status-less transport drops", () => {
    ...
    expect(
      classifyLlmError(new Error("terminated before response"), {
        message: "terminated before response",
      }),
    ).toMatchObject({
      kind: "retryable",
...
const policy = {
  retries: 2,
  timeout: 0,
  backoff: { initial: 100, factor: 2, max: 1000 },
  validationRetries: 0,
};
...
describe("decideRetry", () => {
  ...
  it("retries a transient error with exponential backoff", () => {
    ...
    expect(decideRetry(err, normalized, 0, policy)).toMatchObject({
      kind: "retry",
      reason: "serverError",
      delayMs: 100,
    });
  ...
  it("terminal errors surface as-is", () => {
    expect(decideRetry(new Error("400"), { status: 400, message: "400" }, 0, policy).kind).toBe(
      "terminal",
    );
  });
});
