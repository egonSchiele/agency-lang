...
node main() {
  ...
  match (r) {
    ...
    const errs = typeCheck(
      parsed.result,
      { typechecker: { matchExhaustiveness: "warn" } },
      info,
    ).errors;
    expect(errs.some((e) => e.severity === "warning" && /not exhaustive/i.test(e.message))).toBe(
      true,
    );
    expect(
      errs.some((e) => (e.severity ?? "error") === "error" && /not exhaustive/i.test(e.message)),
    ).toBe(false);
  });
});
...
def f(e: Ev): number {
  match (e) { { kind: "click" } => 1  { kind: "scroll" } => 2 }
        ...
        ERROR,
      ).some((m) => /not exhaustive/i.test(m)),
    ).toBe(false);
  });
...
def f(e: Ev): number {
  match (e) { { kind: "click" } => 1  { kind: "scroll" } if (e.d > 0) => 2 }
        ...
        ERROR,
      ).some((m) => /not exhaustive/i.test(m) && /scroll/.test(m)),
    ).toBe(true);
  });
...
def f(b: boolean): number {
  match (b) { true => 1 }
        ...
        ERROR,
      ).some((m) => /not exhaustive/i.test(m) && /false/.test(m)),
    ).toBe(true);
  });
  it("a `_` arm clears it", () => {
    expect(
