...
if (isInterrupt(__stack.locals.message)) {
  ...
  return { ...state, data: __stack.locals.message };
...
async function _sentiment(message, __metadata) {
  ...
  try {
    ...
    const extracted =  __extractResponse(__rawResult, z.union([z.literal("happy"), z.literal("sad"), z.literal("neutral")]));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  } catch (e) {
    const extracted =  __extractResponse(responseMessage.output, z.union([z.literal("happy"), z.literal("sad"), z.literal("neutral")]));
    console.log(color.yellow("extracted", JSON.stringify(extracted, null, 2)));
    return extracted;
  }
