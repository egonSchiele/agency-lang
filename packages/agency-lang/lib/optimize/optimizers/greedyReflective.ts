...
export class GreedyReflective extends BaseOptimizer {
  ...
  constructor(
    ...
    private readonly greedyDeps: GreedyDeps = {},
  ) {
  ...
  private async attempt(
    ...
    const outcome = await this.proposeValidMutation(
      (diagnostics) =>
        (this.greedyDeps.propose ?? proposeMutation)({
    ...
    const candidate = await this.makeCandidate(
      iter,
      this.fork(),
      preview.targetSet,
      inputs,
    ...
    return {
      ...
      decision,
      rationale: outcome.rationale,
      objective: candidate.scorecard.objective(),
  ...
  private beats(candidate: Candidate, champion: Candidate): boolean {
    ...
    return (
      candidate.scorecard.gatesPassed() &&
      candidate.scorecard.objective() > champion.scorecard.objective()
    );
  }
}
...
function renderHistory(attempts: Attempt[]): string {
  ...
  return attempts
    .map(
      (a) =>
        `- iter ${a.iter} [${a.decision}] objective=${a.objective?.toFixed(3) ?? "n/a"}: ${a.rationale}`,
    )
    .join("\n");
}
