...
Smoltalk client defaults are configured via `AgencyConfig.client` (see `docs/dev/runtime/config.md`) and stored on `RuntimeContext.smoltalkDefaults`. These defaults are merged into every `smoltalk.text()` call and include settings like `defaultModel`, `logLevel`, and API keys.
